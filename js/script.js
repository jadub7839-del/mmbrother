 const statementSection = document.querySelector('.statement');
  ...
  /* ---- Persisted state (cart / wishlist / account) ----
     The site is split across real separate HTML pages now, so state that
     used to just live in memory for the session (cart, wishlist, login)
     now needs to survive that reload. Same data, same shape — the storage
     backend is the only thing that differs:

     - Wishlist and Account use localStorage: a shopper's saved items and
       login stay put even if they close the browser and come back later.
     - Cart uses sessionStorage on purpose: it should still survive normal
       navigation between pages in the same visit (Shop → Product →
       Checkout, back/forward, bfcache restores — all still work exactly
       as before), but a brand new visit — a new tab/window opened after
       the browser was actually closed — always starts with an empty cart,
       instead of resurrecting whatever was left in it from a much earlier
       session. */
  function loadJSON(key, fallback, store){
    try {
  ...
  function saveJSON(key, value, store){
    try {
  ...
  function loadCart(){ return loadJSON('mmbrother_cart', [], sessionStorage); }
  function saveCart(value){ saveJSON('mmbrother_cart', value, sessionStorage); }
