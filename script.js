  ...
  const checkoutLogoBack = document.getElementById('checkoutLogoBack');
  ...
  function openCheckout(){
    ...
    checkoutFormView.style.display = 'grid';
    checkoutConfirmView.classList.remove('show');
    setCheckoutStep(2);
  ...
  checkoutLogoBack.addEventListener('click', (e) => {
    e.preventDefault();
    closeCheckout();
  });
  document.addEventListener('keydown', (e) => {
  ...
  /* Wishlist toggle — click to add, click again to remove */
  ...
  const aboutLogoBack = document.getElementById('aboutLogoBack');

  function openAboutPage(){
    closeAllOverlaysForNav();
    closeShopPage();
    closeProductDetail();
  ...
  aboutLogoBack.addEventListener('click', (e) => { e.preventDefault(); closeAboutPage(); });
  aboutOverlay.addEventListener('click', (e) => { if (e.target === aboutOverlay) closeAboutPage(); });

  /* ---- Shop Page & Product Detail Page navigation ---- */
  const shopOverlay = document.getElementById('shopOverlay');
  const shopClose = document.getElementById('shopClose');
  const shopLogoBack = document.getElementById('shopLogoBack');
  ...
  const pdpLogoBack = document.getElementById('pdpLogoBack');
  ...
  function openShopPage(){
    closeAllOverlaysForNav();
    closeAboutPage();
    if (!shopPageGrid.childElementCount){
      scopedCards.forEach((card, idx) => {
  ...
  function openProductDetail(idx){
    ...
    scopedCards.forEach((otherCard, otherIdx) => {
      ...
      `;
      el.addEventListener('click', () => openProductDetail(otherIdx));
  ...
  shopClose.addEventListener('click', closeShopPage);
  shopLogoBack.addEventListener('click', (e) => { e.preventDefault(); closeShopPage(); });
  shopOverlay.addEventListener('click', (e) => { if (e.target === shopOverlay) closeShopPage(); });

  pdpClose.addEventListener('click', closeProductDetail);
  pdpLogoBack.addEventListener('click', (e) => { e.preventDefault(); closeProductDetail(); });

  document.addEventListener('keydown', (e) => {
    if (e.key !== 'Escape') return;
    if (aboutOverlay.classList.contains('open')) closeAboutPage();
    else if (pdpOverlay.classList.contains('open')) closeProductDetail();
    else if (shopOverlay.classList.contains('open')) closeShopPage();
  });

  document.querySelectorAll('.about-shop-btn').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      closeAboutPage();
      openShopPage();
  ...
  /* Featured Collection / Best Sellers / Latest Drop "SHOP NOW" buttons -> that
     product's Product Detail Page (not connected to the cart directly). */
  scopedCards.forEach((card, idx) => {
    const btn = card.querySelector('.shop-now-btn');
    if (btn){
  ...
  /* Every other "SHOP NOW" / "SHOP" link on the homepage -> the Shop Page. */
  document.querySelectorAll('a[href="#shop"]').forEach(link => {
    if (link.classList.contains('shop-now-btn')) return;
    link.addEventListener('click', (e) => {
      e.preventDefault();
      openShopPage();
    });
